...
if __name__ == "pymol":
    ...
    print("Mean Entropy: " + str(np.max(confidence)))
    #max_conf = max(confidence)
    #confidence = confidence = [x/max_conf for x in confidence]
    # Try plotting z-score.
    conf_mean = np.mean(confidence) 
    conf_std = np.std(confidence)
    confidence  = [(x - conf_mean)/conf_std for x in confidence]
    cmd.alter('all', "b=0.0")
    for i, num in enumerate(numbering):
        cmd.alter("all and resi %d and n. CA"%num, "b=%f" %confidence[i])
    maxval = max(confidence) #np.mean(confidence) - np.std(confidence) 
    minval = min(confidence) #np.log(scipy.stats.entropy(np.repeat(1.00/num_classes, num_classes))/3)
    print(confidence)
    print(maxval)
    print(minval)
